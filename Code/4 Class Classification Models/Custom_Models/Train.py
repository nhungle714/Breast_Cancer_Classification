...
model = CNN_Disease()
model.load_state_dict(torch.load('../../HW/Custom_ModelBonus_Wt',map_location=torch.device('cpu')))
model.fc = nn.Linear(256,4)
...
model, checkpoint_mdl_wt, checkpoint_optim, checkpoint_scheduler, acc_dict, loss_dict = train_model(model,optimizer, scheduler, loss_fn,num_epochs =50)

torch.save(model.state_dict(), 'Model_reg_Wt')
torch.save(checkpoint_mdl_wt,'Model_reg_chkpt_Wt')
torch.save(checkpoint_optim,'Model_reg_chkpt_optim')
torch.save(checkpoint_scheduler,'Model_reg_chkpt_scheduler')
pickle.dump(acc_dict,open("Model_reg_AccDict.pkl","wb"))
pickle.dump(loss_dict,open("Model_reg_LossDict.pkl","wb"))