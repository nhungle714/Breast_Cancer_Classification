...
PlotAccLoss(BestCNN, 'Custom CNN', 'LossDict', 'Loss')
PlotAccLoss(BestCNN, 'Custom CNN', 'AccDict', 'Accuracy')
PlotAccLoss(BestCNN, 'Custom CNN', 'AucDict', 'AUC')
...
##### Inference Resnet18 #######
BestCNN = torch.load(os.path.join(graph_path, 'CNN.pt'))
y_score_CNN, y_target_CNN = inference(BestCNN, dataloaders['test'])
write_list_to_file(os.path.join(graph_path, 'y_score_CNN.txt'), y_score_CNN)
write_list_to_file(os.path.join(graph_path, 'y_target_CNN.txt'), y_target_CNN)
